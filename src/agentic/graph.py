...
def _textify_action(value: Any) -> str:
    ...
    if value is None:
        return "Review Vajra recommendation."
    ...
    if isinstance(value, (list, tuple)):
        return "; ".join(_textify_action(v) for v in value) or "Review Vajra recommendation."
    return str(value).strip() or "Review Vajra recommendation."
...
def _deterministic_report(state: AgentState) -> str:
    ...
    if isinstance(plan, dict):
        ...
        if isinstance(ia, list) and ia:
            action = _textify_action(ia[0])
        elif ia:
            action = _textify_action(ia)
    if mode == "concise":
        return (
...
def audit_node(state: AgentState) -> AgentState:
    ...
    if isinstance(immediate, list) and immediate:
        ...
        first_action = "Review Vajra recommendation."
    audit = digital_logbook.save_event({
